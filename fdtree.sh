...
#
usage="fdtree.sh [-C] [-D] [-R] [-l level_depth] [-d dirs_per_level]
...
# level is the level to create and base is the pathname of the starting point
{
	...
	nd="${dirs_per_level}"
	while [ "$nd" -gt "0" ] && [ "$nl" -gt "0" ]
	do
		if [ "${DEBUG}" -ne "0" ]
		then
			echo "\tcreating dir(\"${nd}\",\"${base_name}L${nl}D${nd}/\")"
		fi
	...
	do
		if [ "${DEBUG}" -ne "0" ]
		then
			 echo "\trecursing create_dirs(\"$((nl-1))\",\"${base_name}DIR${nd}/\")"
		fi
...
# level is the level to remove and base is the pathname of the starting point
{
	...
	# recurse over subdirectories
	nd=$dirs_per_level
	while [ "$nd" -gt "0" ] && [ "$nl" -gt "1" ]
	do
		if [ "${DEBUG}" -ne "0" ]
		then
			echo "\trecursing remove_dirs(\"$((nl-1))\",\"${base_name}DIR${nd}/\")"
		fi
...
# recursion
{
	...
	do
		...
		if [ "${DEBUG}" -ne "0" ]
		then
			echo "\tcreating file(\"${nf}\",\"${file_name}\")"
		fi
	...
	do
		if [ "${DEBUG}" -ne "0" ]
		then
			echo "\trecursing create_files(\"$((nl-1))\",\"${base_name}DIR${nd}/\")"
		fi
...
# recursion
{
	...
	do
		if [ "${DEBUG}" -ne "0" ]
		then
			echo "\trecursing remove_files(\"$((nl - 1))\",\"${base_name}DIR${nd}/)"
		fi
...
levels=4		# 4 levels in tree
dirs_per_level=10	# 10 subdirectories in each directory (fanout)
files_per_dir=10	# 10 files in each directory
fsize=10		# size of files in file system blocks (4096B)
DEBUG="0"		# DEBUG FLAG.  Default is off
CREATE_TREE=0		# Create tree flag
REMOVE_TREE=0		# Remove tree flag
OPTIND=0		# initialize to 0 before getopts call

while getopts ":l:d:f:s:o:CDR" opt
do
	case $opt in
	l ) levels=$OPTARG ;;
	...
	\? ) IFS=''
		echo $usage
		IFS='\ \t\n'
		echo "\t-C create tree only"
		echo "\t-D turns on debugging"
		echo "\t-R remove tree only"
		echo "\t-l is the number of recursive levels below the root node"
		echo "\t-d is the number of directories to create per level"
		echo "\t-f is the number of files to create per directory"
		echo "\t-o is the starting directory pathname"
		echo "\t-s is the file size (in blocks, 4096 for Linux, to create)"
		echo ""
		echo "WARNING: directories and files created increases polynomically with -l."
		echo "Be careful, very careful.  Your filesystem may suffer..."
		echo ""
		echo "EXAMPLE: fdtree.bash -d 1 -l 2 -f 10000 -s 10000"
		echo "\tThis example is a file stress test and creates two levels with one"
		echo "\tdirectory each and 10K files in each directory of size 10K*4K blocks."
		echo "\tThis is 40,960,000B = 40.96 MB of data per file or 2*409,600,000,000B"
		echo "\t= 819.2 GB of data.  You should not attempt this at home..."
		echo ""
		echo "EXAMPLE: fdtree.bash -d 10000 -l 100"
		echo "\tThis is a directory stress test and creates 100 levels with 10K"
		echo "\tdirectories per level."
		echo "\tThis is 10K directories in the first level and (10K)*(10K)"
		echo "\tdirectories in the second level and (10K)*(10K)*(10K) directories"
		echo "\tin the third level and so on.  10K**100 = 10**500 directories at"
		echo "\tthe 100th level.  You should not attempt this at home or work..."
		exit 1 ;;
	esac
done
...
nftot=$(($ndtot * $files_per_dir))

echo "fdtree-"$VERSION": starting at $base"
echo "\tcreating/deleting $levels directory levels with $dirs_per_level directories at each level"
echo "\tfor a total of $ndtot directories"
echo "\twith $files_per_dir files of size $((fsize*4))KiB per directory"
echo "\tfor a total of $nftot files and $((4*$nftot*$fsize))KiB"
...
then
	...
	echo "\tDirectory creates per second = $results"
	...
	then
		...
		results=$(($nftot / $ttot))
		results2=$(($fsize * $nftot * 4 / $ttot))
	fi

	date
	echo "FILE CREATE TIME IN, OUT, TOTAL      = $tin, $tout, $ttot"
	echo "\tFile creates per second      = $results"
	echo "\tKiB per second               = $results2"
fi
...
then
	...
	echo "\tFile removals per second     = $results"
	...
	echo "\tDirectory removals per second = $results"
fi
